...
def solution(message, spoiler_ranges):
    def binary_find(x_list, num):
        low, high = 0, len(x_list) - 1
        while low <= high:
            mid = (low + high) // 2
            if x_list[mid] < num:
                low = mid + 1
    ...
    msg_len = [len(i) for i in msg_list]
    msg_stack = msg_len # 누적합
    for i in range(1,len(msg_len)):
        msg_stack[i] = msg_stack[i] + msg_stack[i-1] + 1
        
    important = set()
    spoiler_idx = set()

    for i in range(0, len(spoiler_ranges)):
        start, end = spoiler_ranges[i]

        
        st_idx = binary_find(msg_stack, start+1)
        end_idx = binary_find(msg_stack, end)
        
        for i in range(st_idx, end_idx + 1):
            important.add(msg_list[i])
            spoiler_idx.add(i)    
    
    print(important)
    no_spo = set()
    for i in range(len(msg_list)):
        if i not in spoiler_idx:
            no_spo.add(msg_list[i])
    print(no_spo)
    answer = len(important-no_spo)
    return answer
